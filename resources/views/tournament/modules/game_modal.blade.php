    ...
    <div class="modal-content">
      {{ Form::open(array('method' => 'PUT', 'route' => ['game.update', $game->id], 'class' => 'form-horizontal')) }}
          ...
          <div class="table-responsive">
            <table class="table table-striped table-condensed table-hover">
              <tr>
                <td class="col-xs-8">
                  @if (empty($only_score))
                    {{ Form::select('home_id', $teams, $game->home_id, array('class' => 'form-control')) }}
                  @else
              ...
              <tr>
                <td class="col-xs-8">
                  @if (empty($only_score))
                    {{ Form::select('away_id', $teams, $game->away_id, array('class' => 'form-control')) }}
                  @else
        ...
        <div class="modal-footer">
          ...
          @if (!empty($game->group) && $game->group->tournament->type == 'DL')
            <button type="button" class="btn btn-default" data-action="remove"><i class="fa fa-trash"></i> Remove</button>
          @endif
